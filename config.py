...
# 2026-08-25 用户指示: 实盘执行预测止盈位;若预测位期望盈利−手续费 ≤ 此值
# (USDT) → 拒单不开仓
DYNAMIC_TP_LIVE_EXECUTION_ENABLED = True
DYNAMIC_TP_LIVE_MIN_NET_EV_USDT = 0.0
...
# 2026-08-25 用户指示"预测到达止盈点概率>60%才下单维持胜率":
# 全局下单门槛——A 信号与 C 提案都必须过(触达概率不足/无预测数据 → 拒单)
ENTRY_MIN_P_HIT_TP = 0.60
# 2026-08-25 用户指示: 预测止盈位的盈利 − 成本 ≤ 此值 → 拒单
# (防"高触达概率但赢面连手续费都不够"的假胜率单)
ENTRY_MIN_TP_NET_USDT = 0.0
