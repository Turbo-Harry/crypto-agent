...
def main():
    ...
    try:
        ...
        print(f"数据获取失败（离线环境正常）: {e}")
        return
    print(f"数据: BTC 日线 {len(btc)} 根, 恐惧贪婪 {len(fng)} 条\n")

    # 验证门价格结构（factor_gate 离线接口）
    closes = [k["close"] for k in btc]
    price_by_date = {}
    for i, k in enumerate(btc):
        d = datetime.fromtimestamp(k["open_time"] / 1000,
                                   tz=timezone.utc).strftime("%Y-%m-%d")
        price_by_date[d] = (i, k["close"])
    ...
    results.append(evaluate(
        "恐惧贪婪(反向)", fng_dates, [-v for v in fng_values],
        "情绪均值回归:恐慌超卖→未来反弹(行为偏差)", price_by_date, 7))
    ...
    results.append(evaluate(
        "恐惧贪婪变化(反向)", fng_dates[1:], fng_chg,
        "情绪拐点:恐慌加剧→短期超卖反弹(行为偏差)", price_by_date, 7))

    dates = [datetime.fromtimestamp(k["open_time"] / 1000, tz=timezone.utc)
             .strftime("%Y-%m-%d") for k in btc]
    ...
    results.append(evaluate(
        "动量7天(反向)", mom_dates, mom,
        "短期反转:急涨后均值回归(行为偏差/流动性冲击恢复)", price_by_date, 7))

    # 均线偏离因子（价格 vs EMA50，反向）
    ...
    results.append(evaluate(
        "均线偏离50(反向)", dist_dates, dist,
        "估值偏离:价格过度偏离趋势中枢→回归(统计套利)", price_by_date, 7))

    # 输出（验证门裁决，全部试验已入 factor_trials 日志）
    print(f"{'因子':<22} {'meanIC':>8} {'t值':>7} {'净价差%':>9} {'裁决':>16}")
    print("-" * 68)
    for r in results:
        print(f"{r['name']:<22} {r['mean_ic']:>+8.4f} {r['ic_tstat']:>7.2f} "
              f"{r['net_spread']*100:>+8.2f}% {r['status']:>16}")
    print("=" * 68)
    print("验证门: walk-forward 折内 IC → t 值(≥3.0 promote / ≥2.0 watch) + "
          "成本扣除 + 去冗余 + 经济逻辑必填")
    print("裁决与全部试验已写入 factor_trials 表（多重检验可追溯）")
