...
## 14. 飞书/会话回复格式（用户可见，禁止 GitHub MD）

用户在飞书或注入会话里看回复时，**`#` 标题、表格、围栏代码块都不会渲染**（见
`docs/reports/pitfalls.md` 2026-08-20 条目）。Agent 回答持仓/状态/看账类问题时：

1. **禁止**：`| 列 | 表 |`、`# / ##` 标题、``` 代码块。
2. **改用**：首行一句结论 + 换行字段（`标签：值`），列表用 `-` 或 `·`。
3. **加粗**：仅飞书 interactive 卡片（`decision.notify`）里 `**数字**` 有效；
   普通会话回复一律纯文本，不用星号装饰。
4. **持仓示例**（会话可直接复制）：
...
· txn_024 KAITO 开多
  入场 0.3437 · 止损 0.3362 · 止盈 0.3588
  435 合约（约 150 USDT）· 现价 0.3446 · 浮盈约 +0.39 USDT
  止损止盈在交易所侧 · 账本=journal=交易所 一致 ✓

近期：已平 23/24 笔 · 总盈亏 +17.74 USDT · 引擎等下一信号
系统：心跳正常 · 未熔断 · 未暂停
```
