...
- 探索后留出：96-SWAP 短窗口中，事后观察到 B-long 且成本≤0.20R 的 490 条净 +0.1983R；明确将其
  标为探索假设后，在此前封存的两套 90 天库按完全相同规则复核。核心 10 币只保留 77 条，净
  -0.1262R、事件聚类下界 -0.3859R、2/5 折为正；Alt 币 328 条净 -0.2518R、下界 -0.3893R、
  0/5 折为正。两个独立时间留出一致否决，故不新增 cost threshold、不启用 B-long。
...
## 2026-08-24 预注册成本、动量与路由过滤器双留出否证

- 为避免围绕 96-SWAP 一天窗口继续事后挑条件，先固定三个有经济含义且不读取结果的过滤器：候选
  总成本 `<=0.25R`、1H/4H 动量均与候选方向一致、shadow 路由选择与策略一致；只评价三个单项及
  其交集，不搜索其他阈值。成本沿现役口径包含双边 taker、滑点和只计不利方向的预期资金费。
- 在封存的 90 天核心 10 币开发集上，A 基线 5,227 条净 EV `-0.6767R`；成本门、动量门、路由门
  分别为 `-0.0778R/-0.6902R/-0.5112R`。最接近零的 `成本+路由` 只有 47 条且仍为
  `-0.0407R`；三条件交集 19 条、`-0.4182R`。B 基线 5,833 条 `-0.6505R`，所有固定组合均负，
  最好也只有 `成本+路由` 的 `-0.2504R`。
- 完全相同规则随后一次性应用于异币种封存集。A 基线 2,498 条 `-0.3004R`；看似最接近的
  `成本+动量一致` 195 条仍为 `-0.0093R`，而开发集同条件为 `-0.0815R`。A 的成本+路由在留出
  为 `-0.3682R`。B 基线 2,920 条 `-0.4262R`，成本、路由与全部组合介于 `-0.3377R` 和
  `-0.4544R`，没有正绝对期望。
...
## 2026-08-24 B 多头一根确认预注册开发集停止裁决

- 冻结规则：只评价已有 `B_breakout` long 候选；等待下一根 15m 完整收线，只有阳线实体且收盘仍严格
  高于原突破入场价才在随后第一根 1m 开盘入场。ATR 固定使用原候选时已知值，1 ATR 止损、2 ATR
  止盈、4h 超时、同分钟止损优先、同币 4h 冷却，并扣现役手续费、滑点和不利资金费。规则与
  开发→后 30 天→BNB/LTC 的封存顺序在看结果前写入只读 evaluator 和测试。
- 核心 8 币前 60 天：原始 B-long 候选 1,387 条；确认 368 条，冷却跳过 533 条，367 条具有完整
  4h/1m 路径。TP/SL/timeout=`103/236/28`，TP-first 28.07%，毛 EV `-0.0538R`、费用后
  `-0.6360R`、同一入场时点聚类 95% 下界 `-0.8191R`；5/5 顺序折全负，8/8 币费用后均负。
- 裁决：`stop_no_promotion`。开发门失败后同 8 币后 30 天与 BNB/LTC 标的留出均保持
  `sealed_not_opened`；不把“多等一根确认”接入扫描、模型、Harness 或订单链。该结果说明确认可以
  降低频率，却没有提高到正的绝对费用后期望；继续微调确认实体、等待根数或冷却长度会变成结果驱动
  搜索，必须另立独立数据和预注册假设后才可进行。

## 2026-08-24 B 对称假突破反转预注册开发集停止裁决

- 冻结规则：复用同一研究裁决器；B-long 后一根 15m 若收阴且收盘回到原突破价下方，则下一分钟开
  short；B-short 镜像为收阳且收回原价上方后开 long。入场后仍固定原候选 ATR、1R/2R、4h、同分钟
  止损优先、同币 4h 冷却与全成本。首次运行因查询漏返回原方向产生 0 资格，审计后作废并补数据库
  边界反例；该次没有打开后续数据，修复详情已追加 pitfalls。
- 修复后核心 8 币前 60 天：原始 long/short 候选 3,112 条，反向确认 1,093 条，冷却跳过 1,026 条，
  1,092 条具有完整路径。TP/SL/timeout=`363/649/80`，TP-first 33.24%；毛 EV `+0.1088R`，但
  费用后 EV `-0.4510R`、事件聚类 95% 下界 `-0.5405R`，5/5 顺序折全负。只有 ADA 约
  `+0.0023R`，其余 7 币均负，不构成可迁移优势。
- 裁决：`stop_no_promotion`，同 8 币后 30 天与 BNB/LTC 均保持 `sealed_not_opened`。假突破方向在
  手续费前有小幅毛优势，但以现役 1 ATR 风险距离执行后不足以覆盖成本；不得据此加杠杆、放宽止损、
  改 maker 假设或只挑 ADA。任何成本回收/更宽风险距离版本都属于新的执行假设，必须另立预注册和独立
  数据，不得接入当前扫描、模型、Harness 或订单链。

## 2026-08-24 B 假突破 2ATR/4ATR 成本机制停止裁决

- 预注册动机：1ATR/2ATR 假突破在核心开发集毛 EV `+0.1088R`、费用后 `-0.4510R`，因此只检验一个
  保持 2:1 且让 1% 风险仓位随止损变宽而缩小的机制版本：同一资格、同一下一分钟入场与 4h 冷却，
  改为 2 ATR 止损/4 ATR 止盈。它不放大单笔风险，也不改变候选方向或选择币种。
- 同一封存开发集结果：1,092 条完整路径，TP/SL/timeout=`180/451/461`，TP-first 16.48%；毛 EV
  `+0.1110R`、费用后 `-0.1689R`、事件聚类 95% 下界 `-0.2651R`。5 个时间折只有第 3 折为正，8 币
...
## 2026-08-25 当前代码历史预训练实跑

- 用只读 `data/market.db` 的确认行情重放 A/B 到独立 `/tmp` 研究库：110 个 SWAP，候选
  5,022 条，连续 4h/1m 标签 2,942 条；没有使用成交记录，也没有写入 paper/live 运行库。
- A/B 各方向均达到 300/60/60 基础样本门，但两组各 61 个预注册因子 validated 均为 0；A/B
  成本后 EV 分别为 `-0.607779R/-0.537796R`。当前 Logistic/CatBoost + 温度校准训练入口因此
  返回 `insufficient_data`，模型制品 0，最终 `stop_no_promotion`。
- 结论：历史候选与标签流水线可用，当前阻塞已从“没下单/没样本”收敛为“没有样本外有效特征且
  成本后负期望”。不绕过因子门强制出模型，不把单月研究样本冒充自然模拟盘验证。

## 2026-08-25 五组预注册机制组合挑战

- 新增 research-only 组合评价器，固定趋势质量、回踩确认、突破确认、市场共振、执行成本五组；逐方向
  同折比较 Logistic/CatBoost，并复用 purged WF、独立温度校准、precision 与成本后 EV 下界。
- 当前 2,942 条历史标签上，A 可评价 8 个方向组、B 可评价 6 个方向组，全部
  `stop_no_promotion`；CatBoost 无一满足替换门。执行成本组因历史盘口缺失不可评价，B 回踩组因策略
  快照不含回踩字段不可评价，均明确 `insufficient_group_data`。
- 最接近的 B-long 市场共振成本后 OOS 95% 下界仍为 `-0.146207R`。组合只进入研究报告，不写模型
  制品、不进入生命周期；同一数据上停止修改组合，避免把预注册检验变成事后搜索。

## 2026-08-25 OKX 模拟盘日内微观入场确认

- 仅对真实 OKX adapter 的 paper 实例启用；FakeAdapter 测试与 live 实例默认不消费该门。15m 仍产生
  结构候选，1H/4H 仍只作环境；候选通过既有 2:1、全成本 EV 与模型门后，再要求最近已收线 1m/5m
  同向、连续多档事件 OFI、60 秒合约主动成交方向、当前点差与 150 USDT 扫单滑点共同确认。
- 订单流至少 10 个盘口事件且最后事件不老于 5 秒，成交流至少 20 笔；点差达到 8 bps、预期滑点达到
  10 bps、订单流陈旧或任一关键特征缺失均空仓。15 分钟振幅达到 1.5% 时仓位乘 0.5，达到 3% 时拒单；
  缩仓只向下影响既有数量，不能扩大 1%/150 USDT/600 USDT 风险预算。
...
- 当前仓库授权仍是仅 OKX 模拟盘；该配置是安全声明，不接入实盘执行路径，不能
  通过修改配置获得真实资金下单权限。
### 2026-08-25 — 模拟盘 K线+订单流动态止盈直接验证

- `PAPER_DYNAMIC_TP_ENABLED=True`：仅模拟盘消费；实盘保持原路径。
- 候选来自预注册 R:R 网格和最近 K 线有利结构位；每个候选独立重算 4h
  bootstrap 首触概率，订单流对 TP/SL odds 只作有界修正。
- 选择成本后 EV 最大且大于零的止盈位；订单流覆盖不足、预测失败或全部候选
