...
### 2026-08-24 口头采用 CatBoost 却回避依赖会产生名不副实的 Challenger
- 现象：提出“浅层 CatBoost Challenger”后又试图用无新依赖的自研非线性模型替代，名称、实现与验收
  对象不一致，无法证明真正 CatBoost 是否优于 Logistic。
- 根因：只考虑安装体积，没有把重依赖成本与模型真实性分开决策。
- 修复：明确引入 `catboost>=1.2,<2`；训练路径延迟导入，浅树固定种子；CatBoost 与 Logistic 使用同一
  purged walk-forward 折，只有自身过全部旧门且多分类 Brier 相对改善至少 2% 才成为制品。二进制模型
  写入 artifact，消费端按摘要缓存、损坏时 fail-closed。
- 预防：凡方案点名具体框架，要么真实引入并验证，要么明确称为替代模型；不得混用名称。

### 2026-08-24 首模治理门过早阻断模拟盘真实成交采集
- 现象：无 active 入场模型时，严格 2:1 前置门拒绝全部模拟订单；反事实候选仍可训练，但自然模拟
  平仓、真实成交费用和执行质量永远无法在冷启动期增长，形成产品目标上的自相矛盾。
- 根因：把成熟模型的生产资格门直接复用于 paper 冷启动，没有把“训练反事实数据”和“真实执行验收
  数据”分成两条同时采集的证据线。
- 修复：按用户明确授权增加仅真实 OKX paper 可用的 baseline bootstrap；只覆盖
  `no_validated_active_model`，不把审计伪装为 passed，且不覆盖几何、成本、负 EV、额度、冷却、分数、
  风控、Harness/AI 否决、名义上限或交易所侧止损。Fake 与 live 永久不取得该权限。
- 预防：冷启动生命周期设计必须逐项证明每个验收指标有可达的数据生成路径；成熟门不得切断自身所需
  的真实执行证据。

### 2026-08-24 总成熟度把首模启动显示成“必须先有实单”的循环死锁
- 现象：入场模型为空时，`/research/readiness` 同时列出自然模拟盘平仓 60 笔和模型门，用户合理地
  读成“没有模型不下单、没有下单又不能生成模型”。
- 根因：接口把首模 bootstrap、激活后 paper 验收和增强能力成熟度压成一个
  `statistically_complete/blockers`；实际训练和 shadow 晋升使用拒单前冻结的候选及 4h 真实路径标签，
  并不依赖成交，但接口没有表达这个阶段关系。
- 修复：保留原总成熟度兼容字段，新增 `phases`，分别报告 model bootstrap、激活后 paper 验收、
  增强成熟度和当前下单模型资格；不修改任何阈值、生命周期或 fail-closed 开仓门。
- 预防：包含因果先后关系的成熟度不得只给一个扁平 blocker 列表；必须区分前置启动条件与启动后验收。
...
### 2026-08-24 接入共同 Harness 不等于冻结了同等证据面

- 现象：生产库中 A_pullback 206 条候选均有点差、预期滑点和盘口证据，B_breakout 112 条候选这三类
  证据以及订单流却全部为空；两类候选虽然进入同一 Harness，B 仍无法核验流动性风险。
- 根因：B 的 enrich 链只冻结 K 线、资金费和横截面字段；共享 Context 的字段形状掩盖了策略级全空值，
  既有测试也只检查 key 存在，没有检查交易所提供盘口时值是否真正落入冻结 payload。
- 修复：只在 B 已形成自然突破候选后通过 ExchangeAdapter 读取同轮盘口，复用 A 的纯函数计算点差、
  预期滑点、盘口失衡与动态订单流，并合并实时订单流质量字段；取数失败显式留空，不影响 shadow 留样，
  不授予执行权限。
- 预防：共同 Harness 的每个策略都必须独立审计关键特征的非空覆盖率；端到端测试同时验证实际非空值、
  零订单和失败时显式缺失，不能用统一 schema 或字段存在性推断证据完整。

### 2026-08-24 复用特征公式不代表可以复用相邻快照状态

- 现象：B 补盘口后复用了 A 的 `_factor_book_state`；同币同轮若 B 先触发、A 后触发，A 会把 B 刚写入的
  同一快照当作“上一快照”，从而把动态 OFI 误算为零并污染 A 的入场证据。
- 根因：盘口/OI 转换公式是纯函数，可跨策略复用；但 previous snapshot 是策略采样序列的一部分，具有
  策略身份和采样频率，错误地把代码复用扩大成了状态复用。
- 修复：B 使用独立 `_strategy_b_factor_book_state` 与 `_strategy_b_factor_oi_state`；A 的原状态完全不写。
  B 同时冻结独立 OI 变化、价格交互和 basis，首个快照无前值时保持显式缺失。
- 预防：凡因子依赖前值、窗口或缓存，状态 key 至少包含策略身份和标的；测试必须设置 A 哨兵状态并验证
  B 执行前后不变，不能只验证最终因子有值。

### 2026-08-24 过滤后均值为正不代表保留候选稳定可交易

- 现象：自然 A-long 用冻结 8/10bps 流动性门后，61 条保留候选费用后均值 +0.22R，朴素增量下界也
  为正；但按同一 15m 事件聚类后保留组下界变成 -0.18R，5 折仅 2 折同时保留正 EV 与正增量。
- 根因：跨币候选共享同一行情事件，逐行当独立样本低估方差；少数早期折和币种正贡献掩盖后两折保留组
  已转负。流动性门能移除一批差交易，不等于剩余方向信号已有稳定 alpha。
- 修复：同时报告朴素均值、事件聚类保留组绝对下界、增量下界、联合时间折与币种集中度；任一失败即
...
### 2026-08-24 有共同成熟门不代表每条策略都已接入 Harness

- 现象：C 当前身份持续产生自然候选，readiness 也展示独立 Harness `100/30` 门，但数据库中 C 的
  `agent_runs` 永远为 0；A/B 的风险 critic 接线没有覆盖主动提案的独立留样路径。
- 根因：C 在 `run_proposal_cycle` 的 recorder 内写入共同 `signal_samples`，调用方只保存提案与 4h 标签，
  没有在提案几何通过后调用 `_run_harness_shadow`。统计契约与实际生产者没有端到端反例。
- 修复：本轮新冻结且几何有效的 C 提案立即走同一 Harness，策略身份仍为 C 且固定
  `allow_veto=False`；deduplicated 历史周期不补跑，避免用当前新闻/账户污染旧时点。回归同时断言
  C Trace 完成、零订单、零执行权限和重复周期不重复计费。
- 预防：新增策略成熟门时必须从候选生产者追到 run、evaluation、outcome 和 lifecycle；“接口能展示门”
  不能证明样本生产链存在。历史影子补跑还必须验证时点上下文，缺失时宁可不补。

### 2026-08-24 概率收缩不能替代独立概率校准

- 现象：入场模型已有 Beta 先验收缩，但没有独立校准段和温度参数，模型置信度无法证明与真实频率一致。
- 根因：purged walk-forward 只有训练/测试两段；若直接在训练段或测试段拟合温度，会分别产生拟合偏差
  或评价泄漏，最终制品若再用全部样本重训也会破坏校准器对应的基础模型。
- 修复：每个训练折按时间留出尾部 20%（至少 30 条）作为独立校准段，并 purge 标签窗口重叠样本；
  Logistic/CatBoost 只在前段拟合，温度只在校准段拟合，测试段只评价。最终制品沿用同一三段规则，
  校准不足或失败不写模型；推理顺序固定为原始概率→Beta 收缩→温度缩放→成本后 EV。
- 预防：任何概率校准器必须能证明 fit/calibration/test 三者时间隔离；制品记录温度、样本数和校准前后
  LogLoss，生命周期门至少要求 4/5 折具备有效独立校准，不能用“代码里有校准公式”代替证据。

### 2026-08-25 单因子全拒不等于可以事后穷举组合

- 现象：单因子 validated=0 后，容易把“可能存在联合非线性”理解为可遍历所有二阶/三阶组合直到找到
  正结果；这会让同一历史窗口同时承担假设生成、选择和验证。
- 根因：组合候选宇宙随结果扩张，多重检验与选择偏差没有进入门槛；树模型的拟合内交互重要性也会被
  误当作样本外经济证据。
- 修复：看结果前只冻结五个有机制依据的组，逐方向用完全相同的 purged WF、独立校准、precision 和
  成本后 EV 下界评价；结果只写 research-only 报告，不直接生成生命周期制品。
- 预防：组合成员、方向、模型族和停止门必须在运行前冻结；任何看完结果后的换成员、加阈值、挑币种
  都是新假设，必须换独立时间数据，不能复用本轮测试段。

### 2026-08-25 波动率冷启动成功不代表连续入场门长期可用

- 现象：ccxt.pro 后端启动时能用 REST 预热 `vol_15m`，但一分钟后新鲜度门会移除该字段；若实时 ticker
  只更新价格而不维护滚动波动窗口，paper 最终确认将长期以 `volatility_missing` 拒单。
- 根因：原生 WS 后端维护了 15 分钟 `price_win`，ccxt.pro 后端只实现一次性预热，两套同契约后端的
  状态更新语义不一致；同时单个成交批次也可能被误当成“连续成交确认”。
- 修复：ccxt.pro 冷启动把已收线 1m close 注入 15 分钟窗口，随后每个 ticker 持续滚动更新波动率和
  时间戳；合约成交采用 60 秒窗口，并要求至少 20 笔后才允许解释 taker 方向。缺失、陈旧或稀疏均
  fail-closed。
- 预防：实时特征验收必须覆盖“启动即就绪”和“运行超过 freshness TTL 仍就绪”两种状态；连续流特征
  必须同时校验方向、窗口和最小事件数，不能只看一个比例字段存在。

### 2026-08-25 历史样本存在不代表当前策略身份可以读取

- 现象：冻结的 90 天样本按 2:1 切出前 60 天后仍有 7,391 条完整标签，但当前训练器返回 `n=0`。
- 根因：训练器只查询当前 `research_scope_version`；样本绑定生成时的旧策略身份，近期配置身份变化后两者
  不再相等。把旧行直接更新为当前版本会伪造 provenance，不能作为修复。
- 修复：概率训练与行加载增加显式 `strategy_version` 研究参数；默认仍严格使用当前身份。显式历史身份
  进入数据查询、制品签名和制品字段，旧样本不重标、不获得当前运行时权限。
- 预防：历史重放训练必须同时保存并显式传递冻结策略身份；`n=0` 先区分“无物理样本”和“身份不匹配”，
  禁止通过改库版本或移除 scope 条件制造训练样本。

### 2026-08-25 保存了 MFE/MAE 不代表训练目标已经成本感知

- 现象：权威路径表已有 MFE/MAE、触达时间和逐候选成本，但训练行只暴露 TP/SL/timeout；分类器学习
  TP-first，成本只在预测后的 EV 过滤中出现。
- 根因：标签采集与模型消费演进不同步，把“数据库有字段”误当成“训练器已使用字段”。
- 修复：研究行显式增加净收益、MFE/MAE 与触达时间；增加独立校准段冻结阈值的费用后正效用
  meta-label 挑战者，并继续用未见时间折评价。
- 预防：每次新增标签都要审计采集、加载、训练目标、评价和制品权限五段；research-only 挑战者不得因
  单次正结果自动写模型或绕过现有概率、稳定性和风险门。

### 2026-08-25 盈亏比与固定 R 障碍不能混为一个参数

- 现象：讨论“放开 2:1”时容易把 reward:risk 比例误写成绝对 `2R/-1R`，进而无法区分是止损 ATR
  尺度、目标比例还是最大持仓时间导致结果失败。
- 根因：现役 1ATR 止损与 2:1 比例恰好产生 2ATR 目标，两个独立维度在单一基线中数值耦合。
- 修复：研究网格分别保存 `stop_atr_mult`、`reward_risk` 和推导的 `target_atr_mult`；同一路径并行
  结算并按各自风险距离重新换算成本 R，生产配置不变。
- 预防：退出研究必须同时报告止损尺度、盈亏比、目标距离、timeout 和费用后 EV；禁止只用“2R策略”
  这种含混名称，也禁止看完结果后只挑最接近零的组合。

### 2026-08-25 组内权重训练不能用同折预测再训练二层模型

- 现象：把每组模型在自身训练样本上的预测作为二层输入，会让组合器看到拟合内置信度；即使外层最后
  有测试折，二层权重仍从带偏的组概率学习。
- 根因：普通两层 stacking 缺少组模型拟合、二层拟合、策略阈值校准三者的时间隔离。
- 修复：外层训练窗再顺序拆成基础组拟合、stack拟合、policy校准三段，并 purge 4h 标签边界；二层
  权重非负归一，测试段仅评价，同时报告等权和连续信号同覆盖基线。
- 预防：权重必须逐折报告均值、极差、全零折和有效折；权重跨折极差超过 0.50 或任一折全零即拒绝，
  不得用全样本重训后的单组权重图冒充稳定性。

### 2026-08-25 网页 DOM 不能冒充交易所行情事实源

- 现象：希望通过 Chrome 扩展实时读取 OKX 前端内容并交给 Agent 判断，容易把页面文字当成稳定盘口接口。
- 根因：页面存在渲染延迟、虚拟列表、地区差异和随时改版；DOM 字段没有交易所 API 的时间戳与完整性契约。
- 修复：扩展仅在用户主动启用的可见 OKX 标签页采集，排除表单和可编辑内容，哈希去重后写入独立研究表；
  REST/WebSocket 继续作为行情事实源，网页证据没有下单权限。
- 预防：任何浏览器采集必须标注 URL、采集时刻、来源和完整性边界；没有稳定 API 对照时只能作辅助证据。

### 2026-08-25 AI 主动策略不能把“自主方向”和“自主风控”混为一谈

- 现象：把 C 主动提案从 shadow 接入模拟盘时，容易同时取消方向资格、成本门、入场确认和仓位约束，
  使“纯 AI”变成模型输出直接调用交易所。
- 根因：候选发现权限与执行权限没有分开建模。
- 修复：v6 取消代码预先指定 long/short，让模型自主选择方向或空仓；但入场参考、1ATR止损、2:1目标、
  20bps追价限制、1m/5m与订单流确认、成本预测、每日1单和现有仓位/止损硬门继续由确定性代码执行。
  live 模式和非 OKX 适配器永久拒绝。
- 预防：AI策略每次扩大权限必须分别审计候选、几何、成本、额度、风控和执行六层；模型异常默认空仓。

### 2026-08-25 K线适配层返回列表时不能按领域对象属性读取

- 现象：paper最终确认从 `_fetch_klines_any` 得到 `[ts,o,h,l,c,v]`，却读取 `row.ts/row.open`，自然候选
  会因 `AttributeError` 全部失败关闭。
- 根因：辅助函数混淆了交易所领域 `Candle` 与引擎归一后的列表契约，测试只覆盖了对象夹具。
- 修复：最终确认同时兼容字段对象和归一列表，并继续按已收线时间裁剪。
- 预防：跨适配层时间序列测试必须覆盖领域对象与公开归一格式，不能只用一种夹具掩盖契约错位。

### 2026-08-25 命中概率必须绑定具体障碍价位

- 现象：预测摘要只显示 `P(触止盈)`，却没有显示概率所对应的具体止盈价，用户和 AI 都可能把概率误解为任意上涨目标的命中率。
- 根因：预测函数虽然以候选 `tp` 作为首触障碍计算概率，但返回结构没有保留该障碍价位。
- 修复：返回 `expected_take_profit` 并在摘要中紧邻命中概率展示；该值绑定确定性 2R 风控目标，模型不能另造。
- 预防：凡展示障碍命中概率，必须同时展示同一计算口径下的障碍价位和校准状态。

### 2026-08-25 Paper-only 权限不能退化为可配置的 live 开关

- 现象：全量回归发现 live 构造器会在某配置打开时装配 C_AI 主动提案模型，违背仓库仅允许 paper 的授权边界。
- 根因：把不可变的环境授权边界实现成普通策略开关，导致改配置即可扩大到真实资金路径。
- 修复：删除 C_AI live 执行配置；live 不装配主动提案模型，扫描与执行两层继续显式拒绝。
- 预防：真实资金权限必须是代码级失败关闭不变量，不能由热重载配置、模型输出或策略参数开启。

### 2026-08-25 固定 ATR 倍数被误读为业务 2:1

- 现象：训练/开仓审计只比较 TP 与 SL 的价格距离，忽略合约数量及成本，名义 1ATR/2ATR 被称为业务 2:1。
- 根因：没有把每张合约的 ctVal、仓位数量、手续费、滑点和不利资金费统一换算为 USDT。
- 修复：新增 research-only 成本后净 USDT 盈亏比计算和 v37 标签字段；训练器只读取明确标记
  `net-usdt-ratio-v1` 且净比率不低于 2 的样本。旧标签保持 NULL，不能混训。
- 预防：标签生成、训练筛选和开仓审计必须记录 ratio_version；缺少可审计计划数量时不得把 ATR
  倍数或单位化金额冒充实际 USDT，同时也不能把半成品门接入生产造成全量拒单。

### 2026-08-25 AI 信息采集总开关不足以审计实际输入

- 现象：只有 Harness 总开关时，无法独立确认 paper/live 是否采集了预测、盘口、衍生品、新闻或账户信息。
- 根因：模型运行权限与各类输入特征共用一个布尔值，配置不能表达数据最小化和消融实验。
- 修复：在 `config.py` 集中增加模式级和功能级 AI 特征开关；关闭的输入在构造冻结 AgentInput 前移除，
  输出仅按白名单写入 `ai_prediction_features`，并固定 `execution_authority=false`。
- 预防：新增任何 AI 数据源必须同时新增集中配置、输入裁剪测试和落库白名单；live 特征采集永远不能隐含执行权。
### 2026-08-25 — 不要把实盘只读 AI 与实盘 AI 执行混成一个开关

- 现象：`AI_FEATURES_LIVE_ENABLED` 容易被误解为 AI 已获得实盘下单权限。
- 根因：信息读取能力和资金执行权限没有在配置命名上明确分离。
- 修复：增加固定为 `False` 的 `AI_LIVE_ORDER_EXECUTION_ENABLED` 安全声明；实盘
  AI 信息仍只读，执行路径继续硬拒绝。
- 预防：任何 live AI 能力都必须分别审计 data、decision、execution 三个权限面。
### 2026-08-25 动态止盈不能只改挂单价而沿用固定 2:1 概率

- 现象：固定 ATR 止盈被要求立即替换为 K 线与订单流预测点位。
- 根因：既有首触概率、标签和开仓门都绑定固定 2:1；只覆盖 `sig.tp` 会让预测
  障碍与真实挂单障碍不一致。
- 修复：模拟盘对每个市场候选 TP 重新计算同批路径首触概率，以冻结订单流作有界
  方向修正，再按费用后 EV 选优；订单流缺失或无正 EV 时拒单。
- 预防：任何执行障碍变化都必须同步改变预测对象、成本门和落库身份。

### 2026-08-25 成交重锚不能重复应用 stop_adj

- 现象：`stop_adj=0.2` 的兼容路径实际止损距离变成 1.44×ATR。
- 根因：信号价阶段已把止损放宽为 1.2×ATR，成交均价重锚时又乘 `(1+0.2)`。
- 修复：重锚只平移信号中已经确定的止损/止盈距离，不再重复乘调整系数。
- 预防：参数调整与成交重锚分层测试；后者只能改变锚点，不能再次改变几何。
### 2026-08-25 固定 R 网格不是市场生成的最佳止盈位

- 现象：动态 TP v1 在 1R/1.5R/2R/2.5R/3R/4R 中选优，仍带人为锚定。
- 根因：把数值优化的离散搜索便利误当成候选价格的市场依据。
- 修复：v2 删除固定 R 网格；候选仅来自独立发现路径的有利极值和最近确认
  结构价，并在另一批独立路径上评价首触概率和成本后 EV。
- 预防：R 只作为结果归一化单位，不能反向生成目标价；候选来源必须可追溯到
  冻结市场数据。
