...
短窗口事后探索曾发现 `B long + cost≤0.20R` 的 490 条候选净 EV +0.1983R。该条件没有当作
预声明结论，而是立即送入此前封存的两个 90 天库：核心 10 币 77 条净 -0.1262R、事件聚类
下界 -0.3859R、仅 2/5 折为正；Alt 币 328 条净 -0.2518R、下界 -0.3893R、0/5 折为正。
因此它是一天横截面偶然，不是可迁移的低成本 B-long edge；不增加生产成本阈值，也不启用 B。

裁决继续为 `stop_no_promotion`：模型列表保持为空，Harness 仅 shadow，预算不扩大。该扩展横截面
与 90 天时间面得到相同否决方向，加强了“当前准确率不足”的证据，但不等于证明未来不可改善。

## 12. 预注册成本、动量与路由过滤器（2026-08-24）

短窗口低成本 B-long 假设被留出否决后，不再搜索数据驱动阈值。下一轮在看结果前固定三项：总成本
`<=0.25R`、1H/4H 动量同时方向一致、shadow 路由与候选策略一致；只测试单项和逻辑交集。该测试
仍使用现役双边 taker、滑点与只计不利资金费的成本口径。

- 90 天核心 10 币：A 基线 5,227 条、净 `-0.6767R`，固定组合中最好是成本+路由 47 条、
  `-0.0407R`；成本+动量 79 条、`-0.0815R`。B 基线 5,833 条、`-0.6505R`，最好组合仍为
  `-0.2504R`。
- 90 天异币种封存集：A 基线 2,498 条、`-0.3004R`，成本+动量 195 条、`-0.0093R`；成本+路由
  恶化到 `-0.3682R`。B 基线 2,920 条、`-0.4262R`，所有组合仍负，最好为 `-0.3377R`。

两个封存集没有任何同规则保留组获得正的扣费后 EV，故本假设族整体 `stop_no_promotion`。相对减亏
不能替代正绝对期望；不改参数、不激活模型、不把研究样本并入自然模拟盘成熟度。
