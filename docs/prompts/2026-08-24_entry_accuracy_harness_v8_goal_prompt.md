...
1. 冻结完整身份：Prompt v8、DeepSeek 模型、Context、Schema、Retrieval、Tool Policy v6 和价格口径
   任一变化都重新计样本，旧版本只保留审计。
...
- 当前完整 v8/v6 每策略自然成熟样本至少 100，合格 reject 至少 30；Trace、概率和 evidence 覆盖 100%。
...
四项冻结资格前置到第一次请求；它不改变 Prompt v8 的风险任务或任何交易权限。
