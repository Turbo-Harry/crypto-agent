...
| 5 | **依赖图/架构图可机检** | 分层规则写进代码检查脚本，而非只写进文档（本仓：tools/dependency_graph --check）。 |
...
| 知识图谱机检 | ✅ | `tools/dependency_graph --check`（AST 分析，✅ 无违规）+ `docs/architecture/code_graph.md`（mermaid+矩阵） |
