...
def validate(version: str, metrics: Mapping[str, Any], *, db_path: str | None = None) -> dict[str, Any]:
    ok, reason = promotion_ready(metrics)
    return transition(version, "validated" if ok else "rolled-back",
                      reason=reason, metrics=metrics, db_path=db_path)


def activate(version: str, *, db_path: str | None = None) -> dict[str, Any]:
    return transition(version, "active-veto", reason="validated sample gate", db_path=db_path)


def observe(version: str, metrics: Mapping[str, Any], *, db_path: str | None = None) -> dict[str, Any]:
